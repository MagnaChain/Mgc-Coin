...
TX.prototype._getAddresses = function getAddresses(view) {
  ...
  for (let addr of output) {
	console.log("################### output addr : " + JSON.stringify(addr) );
 ...
 * @param {BufferReader} br
 */
//将带有witness的不带有witness的读序列化写一起，原来的代码通过一个字节为0判断
//tx是否带有witness不太准确，因为vin数组的大小可能比较大，一个字节表示不下
...
/*
  ...
  return this;
  */
  console.log("################### Bcoin TX fromReader" );
  let flag = 0;
  let witness = 0;
  ...
  br.start();
  this.version = br.readU32();
  console.log("################### Bcoin TX fromReader tx version: " + this.version);
  
  count = br.readVarint();
  console.log("################### Bcoin TX fromReader dummny length: " + count);
  for (let i = 0; i < count; i++)
    this.inputs.push(Input.fromReader(br));
  
  if(count === 0)
  {
	console.log("################### tx has witness");
	flag = br.readU8();
  ...
  {
    count = br.readVarint();
	console.log("################### tx read output count: " + count);
    for (let i = 0; i < count; i++)
      this.outputs.push(Output.fromReader(br));
  }
