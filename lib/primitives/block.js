...
function Block(options) {
  console.log("######################## BCOIN  Block constructor ");
  if (!(this instanceof Block))
  {
	console.log("######################## BCOIN  Block constructor this instanceof Block erro ");
...
Block.prototype.fromOptions = function fromOptions(options) {
  ...
  {
	assert(Array.isArray(options.groupSize));
	for(let sz of options.groupSize )
		this.groupSize.push(sz);
  }
  
  if(options.prevContractData)
  {
	  assert(Array.isArray(options.prevContractData));
	  for(contractdata of options.prevContractData)
		  this.prevContractData.push(contractdata);
  }
...
Block.prototype.frame = function frame() {
  console.log("################### Bcoin Block frame ");
  let raw;

  if (this.mutable) {
	console.log("################### Bcoin Block frame this.mutable is true");
    assert(!this._raw);
    return this.frameWitness();
  }

  if (this._raw) {
	console.log("################### Bcoin Block frame this._raw is not null");
    assert(this._size > 0);
...
Block.prototype.format = function format(view, height) {
  console.log("################### Block.prototype.format " );
  ...
  return {
    ...
    prevHash: util.revHex(this.prevHash),
    merkleRoot: util.revHex(this.merkleRoot),
	hashMerkleRootWithData: util.revHex(this.hashMerkleRootWithData),
	hashMerkleRootWithPrevData: util.revHex(this.hashMerkleRootWithPrevData),
    ...
    bits: this.bits,
    nonce: this.nonce,
	outpointhash: util.revHex(this.outpointhash),
...
Block.prototype.fromReader = function fromReader(br) {
  console.log("################### Block.prototype.fromReader " );
  let witness = 0;
  let count;
  ...
  count = br.readVarint();
  console.log("################### Block fromReader tx count " + count );
  ...
  var groupSizeNum = br.readVarint();
  console.log("################### Block fromReader groupSizeNum " + groupSizeNum );
  for (var j = 0; j < groupSizeNum; j++) {
    this.groupSize.push(br.readU16());
  }
  
  var prevContractDataNum = br.readVarint();
  console.log("################### Block fromReader prevContractDataNum " + prevContractDataNum );
  for (var k = 0; k < prevContractDataNum; k++) {
  }
  
  if (!this.mutable) {
    this._raw = br.endData();
	//var testBlock = Block.fromRaw(this._raw);
...
Block.prototype.fromRaw = function fromRaw(data) {
  console.log("######################## BCOIN  Block fromRaw ");
...
Block.fromReader = function fromReader(data) {
  console.log("######################## BCOIN  Block.fromReader ");
...
Block.prototype.frameWitness = function frameWitness() {
  console.log("################### Bcoin Block frameWitness");
